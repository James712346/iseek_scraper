...
async def DatabaseParser(dataSet):
    ...
    if (not graph):
        ...
        lastRow = await Transit.filter(
            graph_id=dataSet["graphid"]
        ).order_by("-DateTime").first().values()
        if lastRow:
            timeThreshold = lastRow['DateTime'].timestamp()
    # Create Transit Model
    if not len(dataSet["data"]):
        return []
    logger.debug(
        f'''First Data Point for graph {dataSet['graphid']}
        \n \t data: {dataSet['data'][0]}'''
    )
    offset = 0
    ...
    for data in dataSet["data"]:
        ...
        try:
            ...
            logger.error(
                f'''{dataSet['graphid']} failed to grab data from iseek,
                one of the datapoint was NaN'''
                )
            logger.error(
                '''\t- Will go to sleep for 30 Seconds to wait for
                Iseek to update their data''')
            sleep(30)
            return [dataSet["graphid"]]
        else:
            if timeThreshold < row[0] \
                    and list(map(type, row[1:3])) == [float, float]:
...
async def start(IseekInstance: Iseek, DatabaseUrl: str, usingConfig=False):
    ...
    if getenv("no_init") != 'true':
        try:
            ...
            logger.error(
                '''Error when creating the Schema
                (can ignore if using mssql, as it usally minor)''',
                exc_info=True
                )
    else:
        IseekInstance.graphs = await Graphs.all().values_list('ID', flat=True)
    async with IseekInstance:
        await IseekInstance.getAllData(
                CustomParser=DatabaseParser,
                flatten=True,
                parseTitles=False
            )
        for graph in ErroredGraphs:
            ...
            if await DatabaseParser(data) == [graph]:
                logger.critical(
                    f'''Database failed a second time to grab data from
                                                                {graph}!''')
